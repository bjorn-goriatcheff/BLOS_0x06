...
q_t term_kb_wait_q[2], term_screen_wait_q[2]; // Signals buffers

pcb_t pcb[PROC_NUM];    // Process Control Blocks
char proc_stack[PROC_NUM][PROC_STACK_SIZE]; // process runtime stacks
mutex_t mutex[4]; // mutexes
int pies;
page_t page[PAGE_NUM]; // page table
int kernel_cr3;		// Kernel space
...
void ProcScheduler(void) {              // choose run_pid to load/run
   ...
   if(run_q.size==0) run_pid=0;		// reset pid to system
   else{
	run_pid=DeQ(&run_q);		// new process
...
int main(void) {  // OS bootstraps
   ...
   MyBzero((char *)&term_screen_wait_q, sizeof(q_t)*2); // Signals buffers
   MyBzero((char *)&term_kb_wait_q, sizeof(q_t)*2); // Signals buffers
  ...
  // Init pages
   ...
   for(i=0; i<PAGE_NUM;i++){
	page[i].addr=(int *)0xe00000 + i*PAGE_SIZE;
	page[i].lru=0;
	page[i].r_pid=-1;
...
if(run_pid == -1) breakpoint();
   ...
   else if(proc_frame_p->event_type==SYSCALL_EVENT){ // call Service
	//reading EAX value -> calling appropriate service handler 
	if(proc_frame_p->EAX==1) ExitHandler(proc_frame_p); // run_pid = -1, no TT
	else if(proc_frame_p->EAX==2) ForkHandler(proc_frame_p);
	else if(proc_frame_p->EAX==4) WriteHandler();
	else if(proc_frame_p->EAX==7) WaitPidHandler(proc_frame_p);
	else if(proc_frame_p->EAX==11) ExecHandler(proc_frame_p);
	else if(proc_frame_p->EAX==48) SignalHandler(proc_frame_p);
	else if(proc_frame_p->EAX==100) GetPidHandler();
	else if(proc_frame_p->EAX==101) SleepHandler(); 
        else if(proc_frame_p->EAX==102){
		if (proc_frame_p->ECX==LOCK) MutexLockHandler(proc_frame_p->EBX);
		if (proc_frame_p->ECX==UNLOCK) MutexUnlockHandler(proc_frame_p->EBX);
	}
	else if(proc_frame_p->EAX==103){
		GetCharHandler(proc_frame_p->EBX);
	}
	else if(proc_frame_p->EAX==104){
