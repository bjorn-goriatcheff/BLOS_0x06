...
typedef struct {
   ...
   int cr3;		     // main table address
   func_p_t sigint_handler;  // sigint handler
   func_p_t sigchld_handler; // sigchild handler
...
typedef struct{		     //Page table
	int* addr;           // int (4B) address
	int lru;             // lru
	int r_pid;           // running pid
} page_t;
